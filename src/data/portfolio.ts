...
export const projectsDb: Record<string, ProjectEntity> = {
  ...
  'reporting-engine': {
    ...
    descriptionAce: 'ACE had a years-long, company-wide reporting problem: every project — EMS, ERP, Laravel-based services, and more — handled reporting independently, resulting in brittle, unreliable, and non-customizable implementations. I proposed a microservice-based architecture with Python 3.14 as the primary reporting engine, leveraging its rich library ecosystem for flexible report generation and data presentation. I solely developed the engine (since nobody had specialization in polyglot application development), keeping the implementation minimal by design — exposing the underlying library APIs directly, requiring zero overhead for maintenance or custom development. Over time, I evolved the engine into a full-scale analytics workspace: orchestrating a Jupyter Notebook runtime directly inside the ERP for custom Python analytics with notebook persistence, sharing, and curated financial visualization presets. Integrated Google Gemini API endpoints for on-the-fly Python code generation inspired by Google Colab\'s generative workflows. Transitioned from rigid JSON schemas to a directed graph schema architecture enabling dataset inheritance — a massive time saver for the accounting team. Added frequency-based snapshot persistence for strict financial audits. I led the engineering and architecture, with implementation workloads steered through Claude Fable and GPT-6 Astra flagship models. As a live demonstration of this methodology, I orchestrated specialist agent squads — frontend, backend, database-schema, and data specialists — that coordinated to piece together each major module\'s bigger picture and the shape of its data, then synthesized the JSON schemas for the reporting presets (spanning Budget Management with multi-currency and distribution engines, and the entire Procurement lifecycle) with incredible precision, and authored a 3,000+ test suite to guarantee unbreakable reliability. The engine is now integrated across EMS, ERP, and multiple Laravel-based projects, becoming ACE\'s internal industry standard. The accounting team, the treasury team, their heads, managers, our manager, the CEO, and the CTO all praised the reporting engine, calling it a "marvel of engineering" and a true feat of engineering at ACE. The need for dynamic, visual query construction during this project also gave birth to SchemaFlow — now an open-source standalone tool.',
    ...
    architecturalHighlights: [
      ...
      'Specialist agent squads for module presets — frontend, backend, DB-schema, and data specialists coordinated to synthesize precise JSON schemas for Budget Management (multi-tenant, multi-currency, distribution engine) and Procurement lifecycle templates',
    ...
    impactAce: [
      ...
      'Orchestrated specialist agent squads (frontend, backend, DB-schema, data) to synthesize precise preset schemas for Budget Management and Procurement lifecycle templates'
    ]
  },
  'overwatch': {
    id: 'overwatch',
...
export const experiencesDb: Record<string, ExperienceEntity> = {
  'ace': {
    ...
    highlights: [
      ...
      'Scaled AI integration company-wide: multiple internal orchestrated workflows (incident → ticket → resolution → report → human review → test suite → deployment) and Python-based document analysis, reporting, and dashboard tooling eliminating manual data engineering',
      'Provisioned MCPs enabling the CEO to run internal AI orchestrations himself through his own Claude agent',
...
export const skills = {
  ...
  ai: {
    ...
    description: "For the past 2+ years, I have pushed AI-driven engineering to its absolute vanguard. While I rigorously focus on core software engineering, security, architecture, and UI/UX, I command fleets of frontier AI agents (Claude Fable, GPT-6 Astra) as specialist squads — for instance, in building the ACE Reporting Engine's module reporting presets, a frontend specialist, a backend specialist, a database-schema specialist, and a data specialist coordinated together to assemble each module's bigger picture and the shape of its data, synthesizing precise JSON schemas for Budget Management (multi-tenant, multi-currency) and Procurement lifecycle templates. I am notoriously strict about quality—coupling multi-agent orchestration with comprehensive automated test suites (such as my 3,000+ test case harness for the ACE Reporting Engine) to ensure unyielding production reliability. At ACE Money Transfer, after successfully pitching AI workflows to the PMs, CTO, and CEO, I accelerated deployment speed up to [ac]100x[/ac] without ever compromising on quality, earning unanimous acclaim from leadership. What began as my initiative has grown into company-wide AI infrastructure: we have established numerous internal orchestrated workflows — spanning the full lifecycle of incident → ticket → raise → resolution → report → review, closed by a final human review, an automated test suite, and deployment — alongside embracing Python's rich library ecosystem for document analysis, generation, reporting, visualizations, and dashboard development, completely eliminating the need for manual data engineering. The orchestrations are so approachable that the CEO routinely operates them himself, through his own Claude agent, using the [ac]MCPs[/ac] provisioned for internal operations. This entire AI foundation — the multi-agent methodology, the orchestrated workflows, and the Python tooling ecosystem — was architected by a single systems engineer by profession, not a dedicated AI engineer, yet recognized across the company as the driving force behind its AI transformation.",
    models: [
    ...
    skills: [
      "Multi-agent orchestration for end-to-end feature implementation (Claude Fable, GPT-6 Astra) — e.g. specialist squads (frontend, backend, DB-schema, data) coordinated to synthesize precise module preset schemas",
